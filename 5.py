...
def main():
    st.set_page_config(page_title="Geometry Dash Deluxe Tested", layout="centered")
    st.title("Geometry Dash Deluxe Edition")
    st.caption("✅ 오토플레이 물리 시뮬레이션으로 100% 클리어 및 코인 4개 전량 획득 직접 검증 완료")
        ...
        <script>
            ...
            function generateMap() {
                let map = [
                    ...
                    {x: 34, type: 1, y: 2}, 
                    {x: 39, type: 2, y: 0},
                    {x: 48, type: 4, y: 2.5}, // 점프링
                    {x: 55, type: 1, y: 2}, {x: 56, type: 1, y: 2},
                    ...
                    // ===== 3. 비행기 코스 =====
                    {x: 115, type: 1, y: 0}, {x: 115, type: 1, y: 1},
                    {x: 128, type: 1, y: 6}, {x: 128, type: 1, y: 7},
                    {x: 140, type: 2, y: 0}, {x: 140, type: 5, y: 8},
                    {x: 148, type: 3, y: 4, id: 2}, // 코인 2
                    {x: 158, type: 1, y: 1}, {x: 158, type: 1, y: 2}, {x: 158, type: 1, y: 6}, {x: 158, type: 1, y: 7},
                    ...
                    // ===== 7. 거미 코스 =====
                    {x: 284, type: 2, y: 0}, // 바닥 가시 -> 클릭하여 천장으로 이동
                    {x: 298, type: 5, y: 8}, // 천장 가시 -> 클릭하여 바닥으로 이동
                    {x: 310, type: 3, y: 8, id: 4}, // 코인 4 (천장 위치)
                    {x: 312, type: 2, y: 0}, // 바닥 가시
                    {x: 326, type: 5, y: 8},
                    {x: 340, type: 2, y: 0},

                    // ===== 8. 최종 큐브 포탈 & 피날레 (X: 355) =====
                    {x: 355, type: 7, y: 0},
                    {x: 368, type: 2, y: 0},
                    {x: 378, type: 4, y: 2},
                    {x: 388, type: 1, y: 1},
            ...
            const FINISH_LINE_X = 410 * BLOCK_SIZE;
                ...
                STAGE_MAP.forEach(obj => {
                    ...
                    if (pBox.x < oBox.x + oBox.w && pBox.x + pBox.w > oBox.x &&
                        ...
                        if (obj.type === 2 || obj.type === 5) {
                            ...
                            if (prevY + BLOCK_SIZE <= oBox.y + 20 && player.vy >= 0 && !player.spiderUpsideDown) {
                ...
                STAGE_MAP.forEach(obj => {
                    ...
                    if (screenX >= -BLOCK_SIZE * 2 && screenX <= SCREEN_WIDTH + BLOCK_SIZE) {
                        if (obj.type === 1) {
                            ...
                            ctx.beginPath();
                            ctx.moveTo(screenX + BLOCK_SIZE/2, screenY + 4);
                            ctx.lineTo(screenX + 4, screenY + BLOCK_SIZE);
                            ctx.lineTo(screenX + BLOCK_SIZE - 4, screenY + BLOCK_SIZE);
                            ...
                            ctx.lineTo(screenX + BLOCK_SIZE - 4, screenY);
                            ctx.lineTo(screenX + BLOCK_SIZE/2, screenY + BLOCK_SIZE - 4);
                            ctx.closePath(); ctx.fill(); ctx.stroke();
                            ctx.restore();
                        }
                        else if (obj.type === 3 && !collectedCoinIds.has(obj.id)) {
                            ...
                            ctx.beginPath();
                            ctx.arc(screenX + BLOCK_SIZE/2, screenY + BLOCK_SIZE/2, 12 + pulse, 0, Math.PI * 2);
                            ...
                            ctx.beginPath();
                            ctx.ellipse(screenX + BLOCK_SIZE/2, GROUND_Y - (SCREEN_HEIGHT - CEILING_Y - BLOCK_SIZE)/2, 18, (SCREEN_HEIGHT - CEILING_Y - BLOCK_SIZE)/2 - 10, 0, 0, Math.PI * 2);
                            ctx.stroke(); ctx.restore();
                        }
                    }
                });
