...
def main():
    st.set_page_config(page_title="Geometry Dash - Mathematically Tuned", layout="centered")
    st.title("Geometry Dash Deluxe Edition")
    st.caption("📐 [물리 계산 보정완료] 사람이 100% 코인 수집 및 클리어 가능한 버전")
    ...
    <body>
        ...
        <script>
            ...
            // [사람 반응속도 및 정밀 계산된 물리 물리 매개변수]
            const GRAVITY = 1.15;
            const JUMP_STRENGTH = -15.2; // 일반 점프 최고 높이 = 약 100px (블록 2.5개 높이)
            const SPEED = 5.5;          // 사람 눈으로 트래킹 가능한 쾌적한 속도
            ...
            function generateMap() {
                // 타입 정의: 1=블록, 2=바닥가시, 3=코인, 4=점프링, 5=천장가시, 6=비행기포탈, 7=큐브포탈, 8=거미포탈
                let map = [
                    // ===== 1. 초반 큐브 (코인 1: 가시 점프 최고점에 정확히 위치) =====
                    {x: 20, type: 2, y: 0}, 
                    {x: 26, type: 3, y: 2.2, id: 1}, // ★ 코인 1: x=20 가시 뛰어넘는 최고점에 배치
                    {x: 38, type: 1, y: 1}, 
                    {x: 48, type: 4, y: 2},          // 점프링
                    {x: 56, type: 2, y: 0},
                    {x: 65, type: 1, y: 1},
                    ...
                    // ===== 3. 비행기 코스 (코인 2: 중앙 통로 비행 중 자연스럽게 획득) =====
                    {x: 105, type: 1, y: 0},
                    {x: 120, type: 1, y: 7},
                    {x: 135, type: 3, y: 4, id: 2},  // ★ 코인 2: 중앙 안정 지대에 배치
                    {x: 150, type: 2, y: 0},
                    {x: 165, type: 5, y: 8},
                    ...
                    // ===== 5. 중간 큐브 코스 (코인 3: 점프링 이용 시 정면 취득) =====
                    {x: 205, type: 2, y: 0},
                    {x: 220, type: 4, y: 2},         // 점프링
                    {x: 227, type: 3, y: 4.5, id: 3},// ★ 코인 3: 점프링 궤적상 정면에 배치
                    {x: 240, type: 1, y: 1},
                    {x: 255, type: 2, y: 0},
                    ...
                    // ===== 7. 거미 코스 (코인 4: 천장 반전 시 획득) =====
                    {x: 295, type: 2, y: 0},         // 바닥 가시 (위로 반전 필요)
                    {x: 310, type: 3, y: 7, id: 4},  // ★ 코인 4: 천장 라인에 배치 (반전 후 획득)
                    {x: 325, type: 5, y: 8},         // 천장 가시 (바닥으로 반전 필요)
                    {x: 345, type: 2, y: 0},
                    ...
                    if (player.isGrounded) {
                        ...
                        STAGE_MAP.forEach(obj => {
                            if (obj.type === 4) {
                                ...
                                if (dist < BLOCK_SIZE * 3.5) { // 점프링 인식 범위 너그럽게 설정
                                    player.vy = JUMP_STRENGTH;
                                    shockwaves.push({
            ...
            function update() {
                ...
                STAGE_MAP.forEach(obj => {
                    ...
                    // ★ 코인 획득 전용 넉넉한 히트박스 (스쳐도 획득)
                    if (obj.type === 3 && !collectedCoinIds.has(obj.id)) {
                        let coinBox = { x: objX - 10, y: objY - 10, w: BLOCK_SIZE + 20, h: BLOCK_SIZE + 20 };
                        if (pBox.x < coinBox.x + coinBox.w && pBox.x + pBox.w > coinBox.x &&
                            pBox.y < coinBox.y + coinBox.h && pBox.y + pBox.h > coinBox.y) {
